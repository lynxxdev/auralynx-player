...
module.exports = {
  ...
  execute(message) {
    try {
      ...
      if (!player || !player?.current) return message.reply({ embeds: [embedMessage("Não há nada tocando nesse servidor.")], ephemeral: true });
      ...
      let nowPlaying = embedMessage()
        .addFields({ name: "Faixa:", value: `[\`${song.title}\`](${song.uri})`, inline: true })
        .addFields({ name: "Escolhido (a) por:", value: `<@${song.requester.id}>`, inline: true })
